...
头像采用正面镜像视角：抬头时模型抬头，向右歪头时模型向屏幕右侧歪，向左转头时模型向屏幕左侧转。坐标转换依据 Apple 耳机运动数据的坐标轴定义（X 指向右耳、Y 指向正前方、Z 指向头顶，见 [官方坐标轴插图](https://developer.apple.com/documentation/coremotion/cmheadphonemotionmanager)），再对正面显示做旋转轴映射 `(-x, -z, -y)`；无需手机摄像头。映射由 `Pose.basis` 实现，方向由 `Tests/PoseTests.swift` 的点头、左转、右歪头三项测试锁定。
...
- 单元测试：组合姿态归零、坐标轴变换、跨 ±180° 的相对旋转、抬头/左转/右歪头三个方向、暂停停止演示更新。
- UI 测试：演示 → 归零 → 数据 → 设置 → 暂停 → 帮助 → 空数据状态，并保存截图到 xcresult。
- 真机验收：运动权限允许/拒绝、AirPods 佩戴与断连重连、左右转头/点头/歪头的物理方向、后台恢复、长时间漂移。这些不由模拟器测试替代。
- 坐标轴约定：Apple 只用插图给出耳机坐标轴（X 右、Y 前、Z 上），文档正文没有文字描述。单元测试验证的是映射几何自洽，佩戴时 +Z 是否确实朝头顶仍需真机确认。
